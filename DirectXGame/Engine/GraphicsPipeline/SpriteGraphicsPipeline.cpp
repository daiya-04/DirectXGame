...
using namespace SpriteParam;

void SpriteGraphicsPipeline::Init() {
	...
	rootParameters[static_cast<size_t>(RootParameter::kMaterial)].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;   //PixelShaderで使う
	rootParameters[static_cast<size_t>(RootParameter::kMaterial)].Descriptor.ShaderRegister = 0;   //レジスタ番号0とバインド

	rootParameters[static_cast<size_t>(RootParameter::kWVPMat)].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;   //CBVを使う
	rootParameters[static_cast<size_t>(RootParameter::kWVPMat)].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;   //VertexShaderで使う
	rootParameters[static_cast<size_t>(RootParameter::kWVPMat)].Descriptor.ShaderRegister = 0;   //レジスタ番号0を使う
	...
	rootParameters[static_cast<size_t>(RootParameter::kTexture)].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL; //PixelShaderで使う
	...
	//Shaderをコンパイルする
	ComPtr<IDxcBlob> verterShaderBlob = dxCompiler_->ShaderCompile(L"Sprite.VS.hlsl", L"vs_6_0");
	assert(verterShaderBlob != nullptr);

	ComPtr<IDxcBlob> pixelShaderBlob = dxCompiler_->ShaderCompile(L"Sprite.PS.hlsl", L"ps_6_0");
