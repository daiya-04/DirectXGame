...
using namespace SkyBoxParam;

void SkyBoxGraphicsPipeline::Init() {
	...
	rootParameters[static_cast<size_t>(RootParameter::kWorldTransform)].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;   //PixelShaderで使う
	rootParameters[static_cast<size_t>(RootParameter::kWorldTransform)].Descriptor.ShaderRegister = 0;   //レジスタ番号とバインド
	...
	//Shaderをコンパイルする
	ComPtr<IDxcBlob> verterShaderBlob = dxCompiler_->ShaderCompile(L"SkyBox.VS.hlsl", L"vs_6_0");
	assert(verterShaderBlob != nullptr);

	ComPtr<IDxcBlob> pixelShaderBlob = dxCompiler_->ShaderCompile(L"SkyBox.PS.hlsl", L"ps_6_0");
