...
void SkinningGraphicsPipeline::Init() {
	...
	rootParameters[static_cast<size_t>(RootParameter::kEnvironmentTex)].DescriptorTable.pDescriptorRanges = descriptorRangForETex; //Tableの中身の配列を指定
	rootParameters[static_cast<size_t>(RootParameter::kEnvironmentTex)].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangForETex); //Tableで利用する数
	...
	rootParameters[static_cast<size_t>(RootParameter::kDeadEffect)].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;  //CBVを使う
	rootParameters[static_cast<size_t>(RootParameter::kDeadEffect)].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;  //PixelShaderで使う
	rootParameters[static_cast<size_t>(RootParameter::kDeadEffect)].Descriptor.ShaderRegister = 4;  //レジスタ番号1を使う
	...
	//Shaderをコンパイルする
	ComPtr<IDxcBlob> verterShaderBlob = dxCompiler_->ShaderCompile(L"SkinningObject.VS.hlsl", L"vs_6_0");
	assert(verterShaderBlob != nullptr);

	ComPtr<IDxcBlob> pixelShaderBlob = dxCompiler_->ShaderCompile(L"SkinningObject.PS.hlsl", L"ps_6_0");
...
void SkinningGraphicsPipeline::preDraw() {
